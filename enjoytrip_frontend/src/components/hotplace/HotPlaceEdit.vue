...
import { getHotplaceDetail, updateHotPlace } from "@/api/hotPlace";
...
const onSubmit = async () => {
  ...
  let result = await updateHotPlace(route.params.id, formData);

  if (result) {
    alert("핫플래이스 수정 완료");
    router.push({ path: `/hotplace/${route.params.id}` });
  } else {
    alert("핫플래이스 수정 오류");
  }
};
...
onMounted(async () => {
  ...
  if (data.img !== null) {
    imageSrc.value = `${import.meta.env.VITE_BACKEND_BASE_URL}/img/hotplace/${data.img}`;
  }
});
</script>
